<template>
  <base-layout page-title="Add Session" page-default-back-link="/classList">
    <form class="ion-padding" @submit.prevent="submitForm">
      ...
      <ion-label position="floating">Classes</ion-label>
        ...
        <ion-item>
          ...
          <ion-textarea rows="2" v-model="notes"></ion-textarea>
        </ion-item>
        <!-- @input="$emit('update:notes', $event.target.value)" -->

        <ion-item>
          <ion-label position="floating">postLessonNotes</ion-label>
          <ion-textarea rows="2" v-model="postLessonNotes"></ion-textarea>
        </ion-item>
      ...
      <ion-list>
        <ion-item v-for="student in studentRoster" :key="student.id">
          <ion-toggle
            ...
            :value="student.id"
            :checked="studentsAttended.indexOf(student.id) != -1"
          ></ion-toggle>
...
export default {
  ...
  setup() {
    ...
    console.log("loadedClass.value class sessionAdd: ", loadedClass.value);
    ...
    const setTime = () => {
      ...
      console.log("hours gotten: ", hours)
      console.log("regularTime.value: ", regularTime.value);
      console.log("pre toISOString nextDayTime: ", nextDayTime);

      nextDayTime = nextDayTime.toISOString();
    ...
    // create session ID
    const sessionId = ref(
      loadedClass.value.id +
        new Date().toISOString() +
        Math.floor(Math.random() * 10001).toString()
    );
    const sessionName =
      loadedClass.value.name +
      " " +
      format(parseISO(dayTime.value), "MM/dd/yyyy hh:mm aaa");
    ...
    const studentAttendanceAddOrRemove = (student) => {
      console.log("addOrRemove studentsAttended: ", studentsAttended.value)
      console.log("addOrRemove student: ", student)
      const index = studentsAttended.value.indexOf(student) 
      console.log("index: ", index)
      if (index == -1) {
        studentsAttended.value.push(student);
        console.log("push");
      }
      else {
        studentsAttended.value.splice(index, 1);
        console.log("remove (splice)");

      }
      console.log("studentsAttendedAddorRemeve studentsAttended: ", studentsAttended.value)
    }
    ...
    const submitForm = () => {
      ...
      const sessionData = {
        ...
        notes: notes.value,
        // level: "intermediate",
        postLessonNotes: postLessonNotes.value,
        dayTime: dayTime.value,
        studentsAttended: studentsAttended.value,
